...
<template>
  <div class="space-y-6">
    <section :class="sectionCls">
      ...
      <p class="text-xs text-accent-muted mb-3">弹窗添加后仅可查看或删除，需修改请删除后重新添加。</p>
      <div class="space-y-2">
        <div
          v-for="(comp, idx) in form.companions"
          :key="idx"
          class="flex items-center gap-2 rounded-lg border border-chat-border bg-chat-bg/50 px-3 py-2"
        >
          <span class="flex-1 min-w-0 py-1.5 text-white truncate" :title="companionDisplayText(comp)">{{ companionDisplayText(comp) }}</span>
          <button type="button" class="p-2 rounded-lg text-accent-muted hover:text-red-400 hover:bg-white/5 shrink-0" title="删除" @click="removeCompanion(idx)">
            <Icon icon="mdi:close" class="text-lg" />
          </button>
        </div>
        <p v-if="!form.companions.length" class="text-sm text-accent-muted py-3">暂无，点击下方按钮添加。</p>
        ...
        <div class="fixed inset-0 flex items-center justify-center p-4">
          <DialogPanel class="w-full max-w-md rounded-xl bg-chat-panel border border-chat-border shadow-xl p-4 focus:outline-none">
            ...
            <div class="flex gap-2 mt-4">
              <button type="button" class="flex-1 py-2 rounded-lg bg-accent text-chat-bg font-medium hover:opacity-90" @click="confirmCompanion()">确认</button>
              <button type="button" class="flex-1 py-2 rounded-lg border border-chat-border text-accent-muted hover:text-white" @click="companionDialogOpen = false">取消</button>
            </div>
          </DialogPanel>
        </div>
      </Dialog>
